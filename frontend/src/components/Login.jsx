...
const validationSchema=Yup.object({
  ...
  password:Yup.string().min(2,'minimum 2 charaacter').required("password is required")
})
const LoginForm = () => {
  ...
  const {mutateAsync,isError,error,isSuccess,isPending}=useMutation({
    ...
    // console.log(values);
    //http req
    mutateAsync(values).then((data)=>{ dispatch(loginAction(data));
  ...
  return (
    <form  onSubmit={formik.handleSubmit} className="max-w-md mx-auto my-10 bg-white p-6 rounded-xl shadow-lg space-y-6 border border-gray-200">
      ...
      {/* Display messages */}
      {isPending && <AlertMessage type="loading" message="Loging in .."/>}
